...
*
*   revision: $Id: //icaclient/release/ica1210macosx_Lora/base/inc/private/cmacro.h#1 $
*
...
/* Important note: Although our wire marshalling code happily defines wiremarshalling functions
 * when either BIGEND or ALIGNMENT_REQUIRED is defined, it turns out that according to this file, 
 * wiremarshalling functions will only ever be called when BIGEND is defined. If just
 * ALIGNMENT_REQUIRED is defined, structures will be copied using memcpy, which will fail for any
 * architecture that is little-endian and has a different structure layout than the Windows
 * compilers. */
...
#define WIRE_PTR(type, localptr) type temp##type##localptr, *localptr
...
#define WIRE_READ(type, localptr, wirebuffer) localptr = &temp##type##localptr; Marshall_Read_##type (wirebuffer, localptr)
...
#define WIRE_PTR(type, localptr) type temp##type##localptr, *localptr
#define WIRE_READ(type, localptr, wirebuffer) localptr = &temp##type##localptr;memcpy(localptr,wirebuffer,sizeof(type))
#else
