...
#define __TRACE_AO(func, class, class_str, feature, feature_str, format, ...)	\
    do { \
            LogPrintf(__FILE__, __LINE__, func, class_str, feature_str, format, ##__VA_ARGS__); \
    } while (0)

#define __TRACEBUF_AO(func, class, class_str, feature, feature_str, buffer, bytes) \
    do {	\
        LogBuffer(__FILE__, __LINE__, func, class_str, feature_str, buffer, bytes); \
    } while(0)

#define __TRACE_AO_ONCE(func, line, class, class_str, feature, feature_str, format, ...)	\
	static dispatch_once_t TOKENPASTE2(onceToken, line);	\
	dispatch_once(&TOKENPASTE2(onceToken, line), ^{		\
		__TRACE_AO(func, class, class_str, feature, feature_str, format, ##__VA_ARGS__); \
	})

#define __TRACEBUF_AO_ONCE(func, line, class, class_str, feature, feature_str, buffer, bytes) \
	static dispatch_once_t TOKENPASTE2(onceToken, line);	\
	dispatch_once(&TOKENPASTE2(onceToken, line), ^{		\
		__TRACEBUF_AO(func, class, class_str, feature, feature_str, buffer, bytes); \
	})

#define __TRACE_ONCE(func, line, class, class_str, feature, feature_str, format, ...)	\
	static dispatch_once_t TOKENPASTE2(onceToken, line);	\
	dispatch_once(&TOKENPASTE2(onceToken, line), ^{		\
		__TRACE(func, class, class_str, feature, feature_str, format, ##__VA_ARGS__); \
	})
...
#define __TRACEBUF_ONCE(func, line, class, class_str, feature, feature_str, buffer, bytes) \
	static dispatch_once_t TOKENPASTE2(onceToken, line);	\
	dispatch_once(&TOKENPASTE2(onceToken, line), ^{		\
		__TRACEBUF(func, class, class_str, feature, feature_str, buffer, bytes); \
	})
...
#define __TRACE_OBJC_ONCE(func, line, class, class_str, feature, feature_str, format, ...) \
	static dispatch_once_t TOKENPASTE2(onceToken, line);	\
	dispatch_once(&TOKENPASTE2(onceToken, line), ^{		\
		__TRACE_OBJC(func, class, class_str, feature, feature_str, format, ##__VA_ARGS__); \
	})

#define __TRACEAO_OBJC(func, class, class_str, feature, feature_str, format, ...) \
    do { \
        LogPrintf_ObjC(__FILE__, __LINE__, func, class_str, feature_str, format, ##__VA_ARGS__);	\
    } while(0)

#define __TRACEAO_OBJC_ONCE(func, line, class, class_str, feature, feature_str, format, ...) \
	static dispatch_once_t TOKENPASTE2(onceToken, line);	\
	dispatch_once(&TOKENPASTE2(onceToken, line), ^{		\
		__TRACEAO_OBJC(func, class, class_str, feature, feature_str, format, ##__VA_ARGS__); \
	})
...
#define TRACE_ONCE(class, feature, format, ...) \
	__TRACE_ONCE(PRETTY_FUNC, __LINE__, class, #class, feature, #feature, format, ##__VA_ARGS__)
...
#define TRACEBUF_ONCE(class, feature, buffer, bytes) \
	__TRACEBUF_ONCE(PRETTY_FUNC, __LINE__, class, #class, feature, #feature, buffer, bytes)

#define TRACE_AO(class, feature, format, ...) \
	__TRACE_AO(PRETTY_FUNC, class, #class, feature, #feature, format, ##__VA_ARGS__)

#define TRACEBUF_AO(class, feature, buffer, bytes) \
	__TRACEBUF_AO(PRETTY_FUNC, class, #class, feature, #feature, buffer, bytes)

#define TRACE_AO_ONCE(class, feature, format, ...) \
	__TRACE_AO_ONCE(PRETTY_FUNC, __LINE__, class, #class, feature, #feature, format, ##__VA_ARGS__)

#define TRACEBUF_AO_ONCE(class, feature, buffer, bytes) \
	__TRACEBUF_AO_ONCE(PRETTY_FUNC, __LINE__, class, #class, feature, #feature, buffer, bytes)
...
#define TRACE_OBJC_ONCE(class, feature, format, ...) \
	__TRACE_OBJC_ONCE(PRETTY_FUNC, __LINE__, class, #class, feature, #feature, format, ##__VA_ARGS__)

#define TRACEAO_OBJC(class, feature, format, ...) \
    __TRACEAO_OBJC(PRETTY_FUNC, class, #class, feature, #feature, format, ##__VA_ARGS__)

#define TRACEAO_OBJC_ONCE(class, feature, format, ...) \
	__TRACEAO_OBJC_ONCE(PRETTY_FUNC, __LINE__, class, #class, feature, #feature, format, ##__VA_ARGS__)
...
#define TRACE_AO(class, feature, format, ...)			do {} while(0)
#define TRACEBUF_AO(class, feature, buffer, bytes)		do {} while(0)
#define TRACE_AO_ONCE(class, feature, format, ...)		do {} while(0)
#define TRACEBUF_AO_ONCE(class, feature, buffer, bytes) do {} while(0)

#ifdef __OBJC__
#define TRACE_OBJC(class, feature, format, ...)			do {} while(0)
#define TRACE_OBJC_ONCE(class, feature, format, ...)	do {} while(0)
#define TRACEAO_OBJC(class, feature, format, ...)		do {} while(0)
#define TRACEAO_OBJC_ONCE(class, feature, format, ...)	do {} while(0)
#endif
